...
class X1Sliders(Resource):
    @api.expect(api.model("X1Sliders", {"slider_values": fields.List(fields.Float)}))
    def post(self):
        global x1_others, x1, base_w
        x1 = torch.cat([torch.tensor(api.payload["slider_values"]), x1_others])
        base_w = torch.tensor(stylegan.pca.inverse_transform(x1))
...
class GenerateTransitionImages(Resource):
    ...
    def post(self):
        ...
        base_w_ = torch.tensor(stylegan.pca.inverse_transform(torch.randn(N_PCA) / 5))
